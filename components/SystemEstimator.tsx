...
export default function SystemEstimator({compact=false,initialType='solar',initialStep=1}:{compact?:boolean;initialType?:SystemType;initialStep?:1|2|3}){
  ...
  return <form className="gx-full-estimator" onSubmit={submit}>
    ...
    {status==='success'?<div className="gx-estimator-success"><span><Icon name="check"/></span><h3>Estimate request received.</h3><p>Green X can now review the indicative sizing and contact you for the technical details required for a proposal.</p><button type="button" onClick={()=>{setStatus('idle');setStep(1)}}>Create another estimate</button></div>:<>
