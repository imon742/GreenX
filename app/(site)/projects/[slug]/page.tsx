...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    <section className="project-hero-v3"><Image src={p.cover_image_url||projectFallbackPhoto(p.category)} alt={p.title} fill priority sizes="100vw"/><div className="project-hero-overlay-v3"/><div className="container project-hero-copy-v3"><div className="breadcrumbs-v3"><Link href="/projects">Projects</Link><span>•</span>{catSlug&&<Link href={`/projects/category/${catSlug}`}>{p.category}</Link>}</div><span className="project-type-badge">{p.demo?'Reference design':'Case study'}</span><h1>{p.title}</h1><p>{p.short_description}</p></div></section>
    ...
    <section className="section-v3"><div className="container project-story-v3"><aside><span className="kicker">Project overview</span><h2>{p.demo?'A reference configuration with real engineering structure.':'From requirement to handover.'}</h2><p>{p.demo?'This page demonstrates how a Green X technical case study is structured without presenting the reference application as a completed client project.':'Project pages combine the commercial story with the technical details clients and procurement teams need.'}</p>{catSlug&&<Link className="arrow-link" href={`/projects/category/${catSlug}`}>More {p.category} work <Icon name="arrow"/></Link>}</aside><article><p className="lead-v3">{p.description}</p><h3>Services delivered</h3><div className="deliverable-grid-v3">{(p.services_provided||[]).map(x=><span key={x}><Icon name="check"/>{x}</span>)}</div><h3>Technical snapshot</h3><div className="spec-table-v3">{Object.entries(p.technical_details||{}).map(([k,v])=><div key={k}><span>{k}</span><b>{v}</b></div>)}</div><Link className="btn btn-primary" href="/contact#quote">Discuss a Similar Requirement <Icon name="arrow"/></Link></article></div></section>

    {media.length>0&&<section className="section-v3 section-soft-v3"><div className="container"><div className="section-heading-row"><SectionTitleLike title="Project gallery" body="Installation, equipment and handover imagery from the project."/></div><div className="gallery-grid-v3">{media.map((m:any)=><div key={m.id}><Image src={m.media_url} alt={m.alt_text||p.title} fill sizes="(max-width:760px) 100vw, 33vw"/></div>)}</div></div></section>}
