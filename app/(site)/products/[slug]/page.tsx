...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    <section className="product-detail-v3"><div className="container product-detail-grid-v3"><div className="product-detail-media-v3"><Image src={p.image_url||'/assets/product-powerstation.svg'} alt={p.name} fill priority sizes="(max-width:900px) 100vw, 48vw"/></div><div className="product-detail-copy-v3"><div className="breadcrumbs-v3 dark-breadcrumb"><Link href="/products">Products</Link><span>•</span>{catSlug&&<Link href={`/products/category/${catSlug}`}>{p.category}</Link>}</div><span className="product-kicker-light">{p.brand||'Project selection'}</span><h1>{p.name}</h1><p>{p.short_description}</p><div className="product-actions-v3"><Link className="btn btn-primary" href={`/contact#quote`}>Request Price & Availability <Icon name="arrow"/></Link>{p.brochure_url&&<a className="btn btn-dark-outline" href={p.brochure_url} target="_blank" rel="noreferrer"><Icon name="download"/> Datasheet / Brochure</a>}</div><div className="product-trust-line"><span><Icon name="check"/> Application review</span><span><Icon name="check"/> Installation support</span><span><Icon name="check"/> Maintenance planning</span></div></div></div></section>
